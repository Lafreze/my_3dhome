...
report={'source':source.name,'sourceSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'sourceBytes':source.stat().st_size,'sourceTriangles':original,'triangles':sum(len(o.data.polygons) for o in meshes),'bytes':out.stat().st_size,'height':1.65,'forward':'-Z','sourceBones':0,'sourceAnimations':[],'runtimeBones':11,'runtimeScale':1.3/1.65,'runtimeHeight':1.3,'textures':'1024px, original PBR colors','gestures':'Tailored sitting morph preserves hip and leg volume; 11 restrained bones retain neutral head and hands-in-pockets silhouette. Continuous floor-to-seat transfer, quiet gaze, listening and thinking.'}
