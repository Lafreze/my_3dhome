...
report={'source':source.name,'sourceSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'sourceBytes':source.stat().st_size,'sourceTriangles':original,'triangles':sum(len(o.data.polygons) for o in meshes),'bytes':out.stat().st_size,'height':1.65,'forward':'-Z','sourceBones':0,'sourceAnimations':[],'runtimeBones':11,'runtimeScale':1.05,'runtimeHeight':1.7325,'textures':'1024px, original PBR colors','gestures':'Q-proportioned runtime legs, spine, shoulders and neutral head; hands-in-pockets silhouette retained. Quiet gaze, listening, thinking and stretch poses.'}
