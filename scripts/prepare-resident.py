...
report={'source':source.name,'sourceSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'sourceBytes':source.stat().st_size,'sourceTriangles':original,'triangles':sum(len(o.data.polygons) for o in meshes),'bytes':out.stat().st_size,'height':1.65,'forward':'-Z','sourceBones':0,'sourceAnimations':[],'runtimeBones':9,'runtimeScale':1.15,'runtimeHeight':1.8975,'textures':'1024px, original PBR colors','gestures':'Procedural leg/spine/head poses; hands-in-pockets silhouette retained. No authored finger/hand animation clips.'}
