...
- 书房：菱格软包工作椅、暖白色皮革实木扶手阅读椅。阅读椅原有换色和座上书本继续可用。
...
后续调整采用暖白软包、浅白橡木与少量米白洞石。椅子保留原始缝线、褶皱与 UV；模型按连通部件分出软包、木架和金属材质，原始三角形总数不变。颜色在渲染时重设，木架与软包使用各自的灰度明暗映射，金属底座保留原始 PBR。

桌面、桌架、床头柜及小边桌共用 `app/furniture-suite.ts` 的白橡木材质。工作台、咖啡厅长桌和餐桌、客厅茶几、藏书室书桌、酒吧圆桌、游戏房棋桌和植物园圆桌均采用圆润倒角与内收桌沿，保持原有杯碟、书本与棋盘的接触高度。咖啡厅长桌使用双木质支撑，小桌使用木柱与米白底座。

木纹使用 [White Oak Veneer](https://polyhaven.com/a/white_oak_veneer)，作者 Jenelle van Heerden，CC0。原始 1K 色彩、OpenGL 法线及粗糙度图保存在 `public/materials/furniture`，随既有应用部署。
