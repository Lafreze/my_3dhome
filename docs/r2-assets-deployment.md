...
房间结构、家具、庭院、画作和大部分表面均为本地程序几何 / Canvas；没有五个房间的 GLB 可以拆包。网络资源按房间安排：首屏书房和共享贴图，展厅模型在进入展厅时解析，咖啡厅贴图进入时加载。共享纹理只请求一次；当前房间成功后用 requestIdleCallback 预取相邻房间的字节，不解析非当前房间模型。节流 / Save-Data 网络不做空闲预取，快速切换会停止后续旧预取任务。
...
房间字节统计包含独立访问该房间所需共享纹理，因此不同房间不能直接相加。角色作为独立可见对象按需追加。
...
从书房继续访问客厅 / 卧室无需额外必需静态资源（空闲时仍会预取相邻房间）；进入展示区追加 3,572,096 字节王冠兔；进入咖啡厅追加 3,760,314 字节专用纹理。三个角色双姿势合计 20,960,672 字节：熊 7,188,488；猫 6,454,584；狐狸 7,317,600。解码器仅在模型声明压缩扩展时加载；现有模型未使用这些扩展。
