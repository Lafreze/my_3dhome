...
起点先按允许房间、再按命名安全节点选择，主理人／猫／机器人依次登记占位，并排除当前在线访客座位和脚下范围。`kuro.life.starts.v1` 只保存上次初始节点；存在其他安全选择时排除上次节点，收藏数据不受影响。每次访问随机选择主理人 24 个、猫 17 个、兔子 17 个、鸟 4 个、机器人 11 个安全节点之一。白兔和小鸟初始仍隐藏，沿用稀有访客、天气、前 10 秒安静和独占事件规则；白兔首次出现位置使用本次预选节点。充电底座位置固定，机器人可从通道或充电位开始。没有修改房间布局、光照、天气、在线访客模型或管理设置。

验证：`scripts/check-life-starts.mjs` 覆盖 150 组种子的安全性、上一访问去重、相同种子可复现、50 组真实访客初始占位优先、落脚防滑、实际路径、暂停、服务端种子与离线回退；`check-life.mjs` 的 986 条路线和 30 分钟模拟通过，原小猫步态与资源 8 项检查通过。浏览器三组刷新（桌面、手机模拟、减少动态效果）均取得不同服务器种子，所有角色起始节点均改变，无页面错误。阻断白兔下载与种子接口后房屋、猫、主理人正常，重试恢复 805,788 B 模型；模拟后台和管理弹窗冻结生活时钟。
